...
CheckLDAPAuth(Port *port)
{
    ...
    if (list_length(port->hba->roles) == 1) {
    	HbaToken *first_role = lfirst(list_head(port->hba->roles));
		if (strcmp(first_role->string, "all") != 0) {
        	ldapuser = first_role->string;
        	port->user_name = first_role->string;
    	}
    }

	r = check_usermap(port->hba->usermap, ldapuser, map_name,
					  pg_krb_caseins_users);
